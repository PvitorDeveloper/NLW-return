...
export function FeedbackContentStep({feedbackType, onFeedbackRestartRequested, onFeedbackSent} : FeedbackContentStepProps){
    ...
    return(
        <>
            <header>
                <button type='button' className="top-4 left-3 absolute text-zinc-400 hover:text-zinc-100" onClick={onFeedbackRestartRequested}>
                    <ArrowLeft weight="bold" className="w-5 h-5"/>
                </button>
                    <span className="text-xl leading-6 flex items-center gap-2">
                        <img src={feedbackTypeInfo.image.src} alt={feedbackTypeInfo.image.alt} className='w-5 h-5'/>
            ...
            <footer className="flex gap-2 mt-2">
                <ScreenshotButton screenshot={screenshot} onSreenshotTook={setScreenshot}/>
                <button type="submit" 
